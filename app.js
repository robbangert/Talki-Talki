...
const APP_VERSION = "v2026-05-10.4";
...
async function startAiPlayback() {
  ...
  for (let index = safeIndex; index < state.activeChunks.length; index += 1) {
    ...
    if (!ok) {
      ...
      setPlaybackState(PLAYBACK_STATES.IDLE);
      setPlaybackStatus("Gestopt");
      setStatus("Voorlezen gestopt door een afspeelfout.");
      syncPlaybackButtons();
      return;
    }
  }
...
async function fetchAiAudioBlob(text, sessionId = state.playbackSessionId, attempt = 1) {
  ...
  try {
    ...
    if (isAbortError(error)) {
      if (state.stopRequested || state.playbackState === PLAYBACK_STATES.PAUSED) {
        return null;
      }
      if (attempt < AI_RETRY_ATTEMPTS) {
        const retryInMs = AI_RETRY_DELAY_MS * attempt;
        setStatus(
          `Audio ophalen duurde te lang. Nieuwe poging over ${Math.round(retryInMs / 1000)} sec...`
        );
        await sleep(retryInMs);
        return fetchAiAudioBlob(text, sessionId, attempt + 1);
      }
...
async function speakWithAi(text, sessionId = state.playbackSessionId) {
  const audioBlob = await fetchAiAudioBlob(text, sessionId);
  if (!audioBlob) {
    return false;
  }
...
function syncPlaybackButtons() {
  ...
  els.pauseBtn.disabled = !isPlaying && !isPaused && !isLoading;
...
function abortPendingTtsFetch(reason = "cancelled") {
